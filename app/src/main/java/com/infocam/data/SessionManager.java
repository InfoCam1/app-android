...
public class SessionManager {
    private static final String NOMBRE_PREFERENCIAS = "InfoCamSession"; // Guardaremos todo en un archivo XML con este
                                                                        // nombre.
    ...
    // Creamos el constructor que pasaremos a "Context" para que pueda acceder a los
    // archivos de la aplicación.
    public SessionManager(Context context) {
        pref = context.getSharedPreferences(NOMBRE_PREFERENCIAS, Context.MODE_PRIVATE);
        editor = pref.edit(); // Necesitaremos también el editor para poder escribir o borrar dichas
                              // preferencias.
    }

    /*
     * Creamos el método que guardará los datos del usuario. Este se lanzará cuando
     * la API confirme que el login es correcto.
     * En ese momento, guardaremos todos los datos para poder disponer de ellos
     * posteriormente.
     */
    ...
    // Cuando necesiteos mostrar los datos del usuario en la interfaz del perfil, el
    // método rescatará los datos guardados y creará un nuevo objeto Usuario.
    public Usuario obtenerUsuario() {
        ...
        // Añadiremos también valores por defecto, para rellenar algún hueco si falta
        // esa información.
        user.setId(pref.getInt(CLAVE_ID_USUARIO, -1));
        user.setNombreUsuario(pref.getString("username", "usuario01"));
    ...
    // El token es como un pasaporte que usaremos para comunicarnos con el servidor.
    // crearemos un Getter para que nos sea más fácil utilizarlo.
    public String getToken() {
        return pref.getString(CLAVE_TOKEN, null);
    }

    // También creremos un método que facilite saber si existe una sesión activa o
    // no.
    public boolean estaLogueado() {
        return pref.getBoolean(CLAVE_ESTA_LOGUEADO, false);
    }

    // Cuando cerremos sesión, los datos del XML se borrarán por completo por
    // seguridad.
